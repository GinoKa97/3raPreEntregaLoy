...
def juegoFormulario(request):
    if request.method == "POST":
        formulario3 = JuegoFormulario(request.POST)
        ...
        formulario3 = JuegoFormulario()
        
    return render(request, "AppCoder/formuJuego.html", {"form3" : formulario3})
...
def resultadoJuego(request):
    if request.GET["juego"]:
        ...
        resuJuego = Videojuegos.objects.filter(nombre__icontains=juego)
        
        return render(request, "AppCoder/resultadoJuego.html", {"juego": juego, "resul" : resuJuego})
